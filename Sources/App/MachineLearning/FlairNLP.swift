...
public extension Sequence where Element: RequirementVersion {
    func computeEmbedding(queueContext: QueueContext, pathToExistingLM: String?, pathToTrainedLM: String, is_forward: Bool = true, max_epochs: Int = 10) throws -> EventLoopFuture<Void> {
        return try saveAsCorpus(queueContext: queueContext, pathToCorpus: embeddingCorpusPath).flatMapThrowing { (_) -> Void in
    ...
    func computeAllModels<T: Tag>(queueContext: QueueContext, tags: [T], pathsToModels: String = "resources/taggers/", pathsToLMs: [String], testSplit: Double = 0.1, devSplit: Double = 0.1, epochs: Int = 150) throws -> EventLoopFuture<Void> {
        ...
        return saveTaggingCorpus(queueContext: queueContext, tags: tags, allAttributes: allAttributes, pathToCorpus: pathToCorpus, testSplit: testSplit, devSplit: devSplit).flatMap { (_) -> EventLoopFuture<Void> in
            ...
            return allAttributes.compactMap { (element) -> EventLoopFuture<Void> in
                ...
                return queueContext.eventLoop.future()
            }.flatten(on: queueContext.eventLoop)
        }
    }
    ...
    func saveTaggingCorpus<T: Tag>(queueContext: QueueContext, tags: [T], allAttributes: [String], pathToCorpus: String, testSplit: Double = 0.1, devSplit: Double = 0.1) -> EventLoopFuture<Void> {
        ...
        return queueContext.application.fileio.write(fileHandle: try! NIOFileHandle(path: finalSubPath + "train.txt"), buffer: ByteBuffer(string: corpi.0), eventLoop: queueContext.eventLoop).flatMap { (_) -> EventLoopFuture<Void> in
            queueContext.application.fileio.write(fileHandle: try! NIOFileHandle(path: finalSubPath + "test.txt"), buffer: ByteBuffer(string: corpi.1), eventLoop: queueContext.eventLoop).flatMap { (_) -> EventLoopFuture<Void> in
                return queueContext.application.fileio.write(fileHandle: try! NIOFileHandle(path: finalSubPath + "dev.txt"), buffer: ByteBuffer(string: corpi.2), eventLoop: queueContext.eventLoop)
            }
        }
    }
    
    func saveAsCorpus(queueContext: QueueContext, pathToCorpus: String, maxStringSize: Int=10) throws -> EventLoopFuture<Void> {
        ...
        if !(Bool(os.path.exists(finalSubPath + "train")) ?? false) { os.mkdir(finalSubPath + "train") }
        return queueContext.application.fileio.write(fileHandle: try! NIOFileHandle(path: finalSubPath + "test.txt"), buffer: ByteBuffer(string: first), eventLoop: queueContext.eventLoop).flatMap { (_) -> EventLoopFuture<Void> in
            queueContext.application.fileio.write(fileHandle: try! NIOFileHandle(path: finalSubPath + "valid.txt"), buffer: ByteBuffer(string: second), eventLoop: queueContext.eventLoop).flatMap { (_) -> EventLoopFuture<Void> in
                return zip(Array(0..<rest.count), rest).compactMap { (element) -> EventLoopFuture<Void> in
                    ...
                    print(path)
                    return queueContext.application.fileio.write(fileHandle: try! NIOFileHandle(path: path), buffer: ByteBuffer(string: element.1), eventLoop: queueContext.eventLoop)
                }.flatten(on: queueContext.eventLoop)
            }
        }
    }
...
public struct Trainer {
    ...
    static func computeLabellingModel(for type: String, corpus: PythonObject, pathToTrainedModel: String = "resources/taggers/", pathsToLMs: [String], epochs: Int = 150) {
        ...
        let fullPaths = pathsToLMs.filter { Bool(os.path.exists($0.addSlashIfNeeded() + "best-lm.pt")) ?? false }
        let embedding_types: PythonObject = PythonObject(fullPaths.map { embeddingsModule.FlairEmbeddings(PythonObject($0.addSlashIfNeeded() + "best-lm.pt")) } + [embeddingsModule.WordEmbeddings(wordEmbeddingType)])
